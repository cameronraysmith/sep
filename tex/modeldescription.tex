...
\end{equation}
whose logical matrix representation, $\mathbf{G}$, we would like to construct. In the first factor, $\mathcal{E}(L) = P^L = \{e^{L}_{\vec{j}} | \vec{j} \in P^{|L|}\}$.
...
We will now exemplify equations and inequalities that need to be satisfied in order to guarantee the sheaf condition for the simple case of three genes that form the simplest nontrivial cycle where inconsistency may arise. Suppose that $L = \{l_1,l_2,l_3\}$, $P = \{0,1\}$, $\mathcal{G} = \{\{l_1,l_2\},\{l_2,l_3\},\{l_3,l_1\}\}$. We define probabilities associated to the gene network-phenotype maps for any set $S$ as $p^S_{\vec{j}}= d_{S}(e^S_{\vec{j}})$. This results in
\begin{eqnarray}
\dist (\mathcal{E}(L)) &=& \left\{ p^L_{\vec{i}} \; \bigg| \; \forall\, \vec{i} \in P^{|L|} \; p^L_{\vec{i}} \geq 0, \; \sum_{\vec{i} \in P^{|L|}} p^L_{\vec{i}} = 1 \right\} = \Delta_7 \subset \mathbb{R}^8\\
\dist (\mathcal{E}(\mathcal{G})) &=& \left\{ p^{O}_{\vec{i}} \; \bigg| \; (\forall\, O \in \mathcal{G}) \; (\forall\, \vec{i} \in P^{|O|}) \;\; p^O_{\vec{i}} \geq 0, \; (\forall\, O \in \mathcal{G}) \sum_{\vec{i} \in P^{|O|}} p^{O}_{\vec{i}} = 1 \right\} = \Delta^{\oplus 3}_3 \subset \mathbb{R}^{12}
\end{eqnarray}
...
\sum_{e \in \mathcal{E}(O),\atop  e|l_1=e_{\{l_1\}}} d_O(e) \,\, = \sum_{e' \in \mathcal{E}(O'),\atop e'|l_1=e_{\{l_1\}}} d_{O'}(e')
\end{eqnarray}
