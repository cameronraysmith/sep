...
$$
\mathbb{L}(\mathcal{G}) \equiv \{\mathbf{V} \in \mathbb{R}^{16} | \text{the constraints in Eq. \ref{eq:pparsys} hold}\}
$$
...
\mathbb{L}(\mathcal{G}) &\equiv \{\mathbf{V} \in \mathbb{R}^{n} | \text{the constraints in Eq. \ref{eq:sheafprob} hold}\}
\end{aligned}
\end{equation}
In order to assess the likelihood for a given $\mathcal{G}$ of the existence of a global section satisfying the sheaf constraints givein in Eq. \ref{eq:sheafprob} we would like to compute the ratio of the volumes of these two polytopes
\begin{equation}\label{eq:volrat}
\frac{\text{Vol}(\mathbb{M}(\mathcal{G}))}{\text{Vol}(\mathbb{L}(\mathcal{G}))}
\end{equation}
this is the quantity plotted for various $\mathcal{G}$ (i.e. GRNAs) in Fig. \ref{fig:ncycvolrat}.

In order to compute the quantity in Eq. \ref{eq:volrat} we use the following algorithm (code implementing this algorithm is available as Appendices):
\begin{enumerate}
\item Determine the image of $\mathbf{G}$ by annihilating the cokernel of $\mathbf{G}$ and setting its basis equal to $0$
