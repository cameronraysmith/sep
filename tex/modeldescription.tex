...
Next, we introduce probabilty distributions.  Recall that, if $(S,\mathcal{B})$ is a measurable space, then a probability measure over $(S,\mathcal{B})$ is a mapping $p \colon \mathcal{B} \to [0,1]$ such that $p(S) = 1$ and $p(\bigcup_{k=1}^\infty X_k) = \sum_{k=0}^\infty p(X_k)$ for any collection $X_k$ of pairwise disjoint elements of $\mathcal{B}$.  Let $\dist((S,\mathcal{B}))$ denote the set of all probability distributions over $(S,\mathcal{B})$.

If $(S, \mathcal{B})$ and $(S',\mathcal{B}')$ are measurable spaces, $d \in \dist((S,\mathcal{B}))$ and $d' \in \dist((S',\mathcal{B}'))$ are probability distributions over these spaces, and $f:S \rightarrow S'$ is a morphism of measurable spaces, we will say that $f$ is compatible with $d$ and $d'$ when, for every pair of measurable sets $e,e' \in \mathcal{B}'$, we have $d(e') d(f^{-1}(e)) = d(f^{-1}(e')) d(e)$.  In other words, the map $f$ preserves ratios of proabilities of events.  In the case where $f$ is a (partial) surjection, compatibility completely determines $d'$ in terms of $d$ and thus $\dist$ is a functor from the subcategory of measurable spaces with partial surjections to transformations on probability distributions.  Specifically, when $f$ is a total surjection, this map corresponds to marginalization, when $f$ is a partial isomorphism, it corresponds to conditionialization, and when $f$ is a general partial surjection, it corresponds to a combination of conditionalization and marginalization.
...
The sheaf conditions form an underdetermined system of linear equations for the putative distribution so their solution will assume the form of a linear subspace.  For a solution to be a bona fide probability distribution, all the probabilities must be positive numbers, which is only possible if the putative marginals satisfy suitable inequalities.

Suppose that $L = \{1,2,3\}$, $P = \{0,1\}$, $\mathcal{G} = \{\{1,2\},\{2,3\},\{3,1\}\}$. An example of normalized contingency tables that correspond to this model are presented in \ref{fig:inconsistentthreecycle}A. Taken as ostensible marginals of a joint probability distribution, this collection of normalized contingency tables satisfies the local consistency conditions in \ref{eq:threecycinequalities}
\begin{equation}
...
\end{aligned}
\end{equation}
that result from substituting the numbers from \ref{fig:inconsistentthreecycle}A into \ref{eq:threecycinequalities}
\begin{equation}
\begin{aligned}\label{eq:threecycnumbers}
p^{12}_{00} &= 1 + 0.1 - 0.1 - 0.4 - 0.1 - 0.4 = 0.1 \geq 0, \\
p^{12}_{01} &= -0.1 + 0.1 + 0.4 = 0.4 \geq 0,\\
p^{12}_{10} &= -0.1 + 0.1 + 0.4 = 0.4 \geq 0,\\
p^{23}_{00} &= 1 - 0.1 - 0.1 - 0.4 = 0.4 \geq 0,\\
p^{23}_{01} &= -0.4 + 0.1 + 0.4 = 0.1 \geq 0,\\
p^{31}_{00} &= 1 - 0.1 - 0.1 - 0.4 = 0.4 \geq 0.
\end{aligned}
\end{equation}
The global consistency conditions, however, impose four additional constraints which can be derived by simple elimination of variables
\begin{equation}
