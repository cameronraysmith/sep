...
\end{align*}
The remaining condition $y \in \mathcal{D}(\mathcal{E}(L))$ from \ref{eq:globalpolytope} states that all the probabilities $p^{123}_{ijk}$ must be positive numbers, which is only possible if the putative marginals satisfy suitable inequalities. These inequalities can be derived by simple elimination of variables resulting in
\begin{equation}
