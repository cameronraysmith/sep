...
If $(S, \mathcal{B})$ and $(S',\mathcal{B}')$ are measurable spaces, $d \in \dist((S,\mathcal{B}))$ and $d' \in \dist((S',\mathcal{B}'))$ are probability distributions over these spaces, and $f:S \rightarrow S'$ is a morphism of measurable spaces, we will say that $f$ is compatible with $d$ and $d'$ when, for every pair of measurable sets $e,e' \in \mathcal{B}'$, we have $d(e') d(f^{-1}(e)) = d(f^{-1}(e')) d(e)$.  In other words, the map $f$ preserves ratios of proabilities of events.  In the case where $f$ is a (partial) surjection and $d(f^{-1}(S')) \neq 0$, compatibility completely determines $d'$ in terms of $d$ and thus $\dist$ is a functor from the subcategory of measurable spaces with partial surjections to transformations on probability distributions.  Specifically, when $f$ is a total surjection, this map corresponds to marginalization, when $f$ is a partial isomorphism, it corresponds to conditionalization, and when $f$ is a general partial surjection, it corresponds to a combination of conditionalization and marginalization.
